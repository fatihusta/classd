...
extern void splitd_main_shutdown( void );
...
int splitd_functions_init( char* config_file )
{
...
int splitd_functions_load_config( splitd_config_t* config )
{
...
json_server_function_entry_t *splitd_functions_get_json_table()
{
...
static struct json_object* _hello_world( struct json_object* request )
{
    ...
    if (( response = json_server_build_response( STATUS_OK, 0, "Hello from splitd" )) == NULL ) {
...
static struct json_object *_get_config( struct json_object* request )
{
    ...
    if (( config_json = splitd_config_to_json( &config )) == NULL ) {
...
static struct json_object *_set_config( struct json_object* request )
{
    ...
    int _critical_section( char* message, int message_size ) {
        ...
        if ( splitd_config_load_json( &config, config_json ) < 0 ) {
        ...
        if (( new_config_json = splitd_config_to_json( &config )) == NULL ) {
            errlog( ERR_CRITICAL, "splitd_config_to_json\n" );

...
typedef struct
{
    ...
    /* The thread that owns the scheduler (only set or read this when you have the mutex) */
    pthread_t thread;
...
void* arpeater_sched_donate    ( void* arg )
{
    ...
    debug( 4, "SCHED: Starting scheduler\n" );

    pthread_t thread = pthread_self();
    if ( pthread_mutex_lock ( &_sched.mutex ) < 0 ) return perrlog_null( "pthread_mutex_lock" );
    if ( _sched.thread == 0 ) _sched.thread = thread;
    else thread = _sched.thread;
    if ( pthread_mutex_unlock ( &_sched.mutex ) < 0 ) return perrlog_null( "pthread_mutex_unlock" );

    if ( thread != pthread_self()) {
        return errlog_null( ERR_CRITICAL, "Scheduler thread already running: %d\n", thread ); 
    }
    ...
    debug( 4, "SCHED: Exiting scheduler\n" );

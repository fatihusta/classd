 ...
 */
static int _nfq_get_conntrack( struct nfq_data *nfad, splitd_packet_t* pkt );
...
{
    splitd_nfqueue_t* nfqueue = NULL;
    
    if (( nfqueue = calloc( 1, sizeof( splitd_nfqueue_t ))) == NULL ) return errlogmalloc_null();
...
splitd_nfqueue_t* splitd_nfqueue_create( u_int16_t queue_num, 
                                                     u_int8_t copy_mode, int copy_size )
{
    splitd_nfqueue_t* nfqueue = NULL;

    if (( nfqueue = splitd_nfqueue_malloc()) == NULL ) {
        return errlog_null( ERR_CRITICAL, "splitd_nfqueue_malloc\n" );
    }

    if ( splitd_nfqueue_init( nfqueue, queue_num, copy_mode, copy_size ) < 0 ) {
        splitd_nfqueue_raze( nfqueue );
        return errlog_null( ERR_CRITICAL, "splitd_nfqueue_init\n" );
    }
...
{
    return splitd_nfqueue_set_verdict_mark( packet, verdict, 0, 0 );
}
...
static int _nfq_get_conntrack( struct nfq_data *nfad, splitd_packet_t* packet )
{

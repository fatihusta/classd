...
static int _get_sysfs_attributes( splitd_uplink_t* uplink );
static int _update_primary_address( splitd_uplink_t* uplink );
static int _update_gateway( splitd_uplink_t* uplink );
...
static int _update_primary_address( splitd_uplink_t* uplink )
{
    if ( uplink->alpaca_interface_id < 1 || uplink->alpaca_interface_id > SPLITD_MAX_UPLINKS ) {
        return errlogargs(); 
    }
...
static int _update_gateway( splitd_uplink_t* uplink )
{
    ...
    int rt_table = SPLITD_IP_RT_TABLE_BASE + uplink->alpaca_interface_id;

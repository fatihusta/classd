...
class ArpEater
  ...
  def display_get_active_hosts( body )
    ...
    body["hosts"].each do |host|
      puts "address: %-15s,enabled: %-7s,passive: %-7s,gateway: %-15s" %  [ host["address"], host["enabled"], host["passive"], host["gateway"] ]
    end
  end
  ...
  def display_get_network_settings( body )
    puts "Retrieved the network settings #{body["network_settings"].inspect}"
  end
end

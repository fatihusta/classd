...
LIBITEMS="untangle-libitem-lite-package untangle-libitem-standard-package"
RACK_NODES="untangle-node-spamassassin untangle-node-phish untangle-node-spyware untangle-node-sitefilter untangle-node-kav untangle-node-clam untangle-node-ips untangle-node-protofilter untangle-node-firewall untangle-node-adblocker"
SERVICE_NODES="untangle-node-cpd untangle-node-support untangle-node-faild untangle-node-splitd untangle-node-policy untangle-node-adconnector untangle-node-openvpn untangle-node-shield untangle-node-boxbackup untangle-node-reporting"
...
if [ ! $? -eq 0 ] ; then
    echo "ERROR: Failed to download libitems successfully - call Untangle"
    exit 1
fi
...
# install oem package
#
echo "apt-get install --yes --force-yes untangle-oem-overbox"
#apt-get install --yes --force-yes untangle-oem-overbox
# because it isn't actually in 8.0 just install the one from this directory
dpkg -i ./untangle-oem-overbox_*.deb
...
for i in $RACK_NODES ; do 
    ...
    if [ ! $? -eq 0 ] ; then
        echo "ERROR: Failed to install libitems successfully - call Untangle"
        exit 1
    fi    

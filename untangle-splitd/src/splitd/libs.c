 ...
 *
 * $Id: libs.c 22253 2009-03-04 21:56:27Z rbscott $
 */
...
static int _load_library( char* lib_path_name, char* lib_file_name )
{
    ...
    splitd_uplink_test_lib_t* lib;

    snprintf( function_name, sizeof( function_name ), "splitd_%s_prototype", l );
    
    int _critical_section()
    {
        splitd_uplink_test_prototype_t function;
        ...
        function = (splitd_uplink_test_prototype_t)dlsym( handle, function_name );
        if ( function == NULL ) {

...
static struct json_object *_get_network_settings( struct json_object* request )
{
    ...
    int _critical_section()
    {
        ...
        if ( arpeater_ae_manager_get_ip_settings( &ip, &settings ) < 0 ) {
            return errlog( ERR_CRITICAL, "arpeater_ae_manager_get_ip_settings\n" );
        }
        ...
        if (( response = json_server_build_response( STATUS_OK, 0, "Successfully retrieved settings" ))
            == NULL ) {
            return errlog( ERR_CRITICAL, "json_server_build_response\n" );
        }
    ...
    if ( settings.mac_addresses != NULL ) free( settings.mac_addresses );
    settings.mac_addresses = NULL;
    
    if ( ret < 0 ) {
        if ( response != NULL ) json_object_put( response );
        if ( settings_json != NULL ) json_object_put( settings_json );
        
        response = settings_json = NULL;
        return json_object_get( _globals.internal_error );
    }
